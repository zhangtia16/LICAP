...
from baseline_models.rgcn import RGCN
...
def main(args):
    ...
    # set the save path
    save_root = 'pretrain/results/' + args.dataset + '_' + args.pretrain_model +  '_pretrain_rgcn/'
    ...
    for cross_id in range(args.cross_num):
        ...
        model = RGCN(in_dim=num_feats, hidden_dim=64, out_dim=1, rel_num=rel_num)
        ...
        for epoch in range(args.epochs):
            ...
            if epoch >= 3:
                t0 = time.time()
            # train
            logits = model(g, features, edge_types)
            ...
            model.eval()
            with torch.no_grad():
                val_logits = model(g, features, edge_types)
        ...
        model.eval()
        with torch.no_grad():
            test_logits = model(g, features, edge_types)  # [node*1]
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='PreGAT-Pretrain-RGCN')
